...
def affine_bn_relu_forward(x, w, b, gamma, beta, bn_param):
    """                                                                          
    Convenience layer that performs an affine - batch norm - ReLU
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        # compute loss: data_loss + reg_loss
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        ############################################################################
        layer_dims = zip([input_dim] + hidden_dims,
                         hidden_dims + [num_classes])
        for i, (input_dim, output_dim) in enumerate(layer_dims, start=1):

            # store weights, bias for layer i
            self.params.update({
                'W%d'%i : weight_scale * np.random.randn(input_dim, output_dim),
                'b%d'%i : np.zeros((1, output_dim)),
            })

            # break early on final layer
            if i == self.num_layers:
                break

            # hidden layer stuff
            if self.use_batchnorm is True:
                # scale, shift params for batch norm
                self.params.update({
                    'gamma%d'%i : np.ones((1, output_dim)),
                    'beta%d'%i : np.zeros((1, output_dim)),
                })
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def loss(self, X, y=None):
        ...
        for layer_i in range(1, self.num_layers + 1):
            ...
            # params for layer
            w = self.params['W%d'%layer_i]
            b = self.params['b%d'%layer_i]

            # forward pass on layer
            if layer_i < self.num_layers:

                # hidden layer: {affine - [batch norm] - relu} 
                if self.use_batchnorm is True:
                    gamma = self.params.get('gamma%d'%layer_i)
                    beta  = self.params.get('beta%d' %layer_i)
                    bn_param = self.bn_params[layer_i-1]
        ...
        for layer_i in reversed(range(1, self.num_layers + 1)):

            # get cache for layer, init params
            cache = caches.pop()
            ...
            if layer_i < self.num_layers:
                ...
                if self.use_batchnorm is True:
                    ...
                    grads.update({
                        'gamma%d'%layer_i : dgamma,
                        'beta%d' %layer_i : dbeta,
                    })
                else:
            ...
            # adjust loss, dw for regularization
            w = self.params['W%d'%layer_i]
